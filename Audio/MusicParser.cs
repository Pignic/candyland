...
public static class MusicParser {
			...
			if (trimmedLine.StartsWith("tempo ")) {
				...
				if (colonIndex > 0) {
					...
					if (!string.IsNullOrWhiteSpace(pattern)) {
						string[] notes = pattern.Split(' ', StringSplitOptions.RemoveEmptyEntries);
						if (notes.Length > 0) {
	...
	private static void ParseChannelRange(string line, Song song) {
		...
		string[] parts = line.Substring(6).Split(' ', StringSplitOptions.RemoveEmptyEntries);

		if (parts.Length < 1) {
			return;  // Need at least channel:waveform
		}
	...
	private static void ParsePan(string line, Song song) {
		...
		string[] parts = line.Substring(4).Split(' ', StringSplitOptions.RemoveEmptyEntries);
		if (parts.Length < 3) {
			return;
		}
	...
	private static void ParseEnvelope(string line, Song song) {
		string[] parts = line.Substring(9).Split(' ', StringSplitOptions.RemoveEmptyEntries);
		if (parts.Length < 5) {
			return;
		}
	...
	private static void ConvertPatternsToNotes(Dictionary<int, List<string[]>> patterns, Song song) {
		...
		foreach (KeyValuePair<int, List<string[]>> kvp in patterns) {
			...
			foreach (string[] noteLine in lines) {
				for (int i = 0; i < noteLine.Length; i++) {
					...
					Note note = new Note(channelId, pitchStr, beatPosition, 1f) {
						...
						// Calculate frequency
						Frequency = NoteToFrequency(pitchStr)
					};
