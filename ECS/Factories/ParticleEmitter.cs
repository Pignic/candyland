...
public class ParticleEmitter {
	...
	public void SpawnBurst(string roomId, Vector2 position, Color color, int count = 10, float speed = 100f, float size = 3f) {
		for (int i = 0; i < count; i++) {
			...
			CreateParticle(roomId, position, velocity, color, size, lifetime: 0.5f);
		}
	}

	public void SpawnBloodSplatter(string roomId, Vector2 position, Vector2 direction, int count = 15) {
		for (int i = 0; i < count; i++) {
			...
			Entity entity = CreateParticle(roomId, position, velocity, bloodColor, size, lifetime: lifetime);
			entity.Set(new Gravity(300f));  // Blood falls
		}
	}

	public void SpawnDustCloud(string roomId, Vector2 position, Vector2 diraction, int count = 8) {
		for (int i = 0; i < count; i++) {
			...
			CreateParticle(roomId, position, velocity, dustColor, size, lifetime: 0.8f, fadeSize: true);
		}
	}

	public void SpawnImpactSparks(string roomId, Vector2 position, Vector2 impactNormal, int count = 12) {
		for (int i = 0; i < count; i++) {
			...
			Entity entity = CreateParticle(roomId, position, velocity, sparkColor, size, lifetime: 0.3f);
			entity.Set(new Gravity(200f));  // Light gravity
		}
	}

	public void SpawnHealingParticles(string roomId, Vector2 position, int count = 10) {
		for (int i = 0; i < count; i++) {
			float angle = (float)(_random.NextDouble() * Math.PI * 2);
			...
			CreateParticle(roomId, position, velocity, healColor, size, lifetime: 1f, fadeSize: true);
		}
	}

	private Entity CreateParticle(string roomId, Vector2 position, Vector2 velocity, Color color, float size, float lifetime, bool fadeSize = false) {
